...
export const WidgetLoadingScreen = ({ organizationId }: { organizationId: string | null }) => {
  ...
  const getVapiSecrets = useAction(api.private.secrets.getVapiSecrets); // Use private namespace to fetch VAPI keys from environment variables
  ...
  useEffect(() => {
    ...
    getVapiSecrets({ organizationId }).then((secrets) => {
      setVapiSecrets(secrets); // Expects { publicApiKey, privateApiKey } | null
      setStep("done");
    }).catch(() => {
      setVapiSecrets(null);
      setStep("done");
